...
function candidateToLead(candidate: CompanyCandidate, campaign: Campaign, sourceJobId: string): Lead {
  return {
    id: createId("lead"),
    ...
    microInsight: "Lead criado a partir da descoberta. Nenhuma inferência de decisor ou contato foi adicionada.",
    suggestedMessage: "Abordagem indisponível até que os dados necessários sejam enriquecidos e aprovados.",
  };
}
...
export class ProspectingService {
  ...
  async createAndRun(input: Campaign): Promise<{ campaign: Campaign; jobs: ProspectingJob[]; leads: Lead[] }> {
    ...
    try {
      ...
      setJobState(discoveryJob, "completed", { processed: candidates.length, total: campaign.quantity, attempts: 1 });
      ...
      return { campaign: updatedCampaign, jobs: jobRepository.listByCampaign(campaign.id), leads };
    } catch (error) {
      ...
      throw Object.assign(new Error("Não foi possível executar o discovery."), { cause: error, campaign: updatedCampaign });
    }
  }
}
